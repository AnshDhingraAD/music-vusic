...
async function fetchsongs(folder) {
    ...
    let songsul=document.querySelector('.songslist').getElementsByTagName("ul")[0]
        ...
        for (const song of songs) {
            ...
            <div class="songinfo">
            <div class="songname">${song.split('songs/')[1].split('/')[1].replaceAll('%20',' ').split('.mp3')[0].split('-')[0]}</div>
        ...
        document.querySelectorAll(".songslist li a").forEach(a =>{
            a.addEventListener('click', element =>{
                ...
                currentaudio.play();
                play.src="pause.svg"
                document.querySelector('.songnaam').innerHTML=currentaudio.src.split('songs/')[1].split('/')[1].replaceAll('%20',' ').split('.mp3')[0].split('-')[0] + "<br>" + currentaudio.src.split('songs/')[1].replaceAll('%20',' ').split('.mp3')[0].split('-')[1]
            });
        }); 
